...
const dht = function(pi, gpio) {
  ...
  return new Promise((resolveDht, rejectDht) => {
    if((callbackId =
      pigpiod.callback(pi, gpio, pigpiod.RISING_EDGE, intrGpioDht)) < 0
    ) {
      throw new Error('Failed to pigpiod.callback()');
    }

    pigpiod.gpio_write(pi, gpio, 0);
    pigpiod.set_mode(pi, gpio, pigpiod.PI_INPUT);
    setTimeout(() => {
      ...
      .then(() => {
        ...
        pigpiod.set_mode(pi, gpio, pigpiod.PI_OUTPUT);
        pigpiod.gpio_write(pi, gpio, 1);
...
module.exports = {
  ...
  DHT_TIMEOUT,
  DHT_BAD_CHECKSUM,
  DHT_TEMP_OUT_OF_RANGE,
  DHT_HUMIDITY_OUT_OF_RANGE,
  dht
};
